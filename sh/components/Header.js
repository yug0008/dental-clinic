        ...
        <div className="container-custom px-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 md:gap-4 overflow-x-auto hide-scrollbar">
              <a href="tel:+15594321300" className="flex items-center gap-1 text-sm whitespace-nowrap hover:text-blue-200 transition-colors">
                <FaPhone className="text-[#01d09e] text-xs" />
                <span className="text-xs md:text-sm">(719) 354-2900</span>
              </a>
              ...
              <div className="flex items-center gap-1 text-sm whitespace-nowrap">
                ...
                <span className="text-xs md:text-sm truncate max-w-[120px] xs:max-w-[200px] sm:max-w-none">
                 7903 Silicon Heights, Colorado Springs, CO, USA
                </span>
              </div>
...
<nav className="hidden lg:flex items-center space-x-6 xl:space-x-8">
  ...
  <Link href="/insurance" className="flex items-center gap-1 text-[#0194d0] hover:text-[#01d09e] transition-colors font-medium whitespace-nowrap">
  ...
  <div className="relative group">
    <button 
      className="flex items-center gap-1 text-[#0194d0] hover:text-[#01d09e] transition-colors font-medium whitespace-nowrap"
      onMouseEnter={() => setMoreDropdownOpen(true)}
      onMouseLeave={() => setMoreDropdownOpen(false)}
    >
      <span>More</span>
      <FaChevronDown className={`text-xs transition-transform duration-300 ${moreDropdownOpen ? 'rotate-180' : ''}`} />
    </button>
    
    {/* Dropdown Menu */}
    <div 
      className={`absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 z-50 ${
        moreDropdownOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'
      }`}
      onMouseEnter={() => setMoreDropdownOpen(true)}
      onMouseLeave={() => setMoreDropdownOpen(false)}
    >
      ...
      >
        <div className="w-8 h-8 bg-[#0194d0]/10 rounded-lg flex items-center justify-center group-hover:bg-[#0194d0] transition-colors">
          <FaEnvelope className="text-[#0194d0] group-hover:text-white transition-colors text-sm" />
        </div>
        <div>
          <p className="font-medium text-gray-900 group-hover:text-[#0194d0] transition-colors">Contact</p>
      ...
      >
        <div className="w-8 h-8 bg-[#0194d0]/10 rounded-lg flex items-center justify-center group-hover:bg-[#0194d0] transition-colors">
          <FaStar className="text-[#0194d0] group-hover:text-white transition-colors text-sm" />
        </div>
        <div>
          <p className="font-medium text-gray-900 group-hover:text-[#0194d0] transition-colors">Testimonials</p>
          <p className="text-xs text-gray-500">Read patient stories</p>
        </div>
      </Link>
      ...
      >
        <div className="w-8 h-8 bg-[#0194d0]/10 rounded-lg flex items-center justify-center group-hover:bg-[#0194d0] transition-colors">
          <FaCreditCard className="text-[#0194d0] group-hover:text-white transition-colors text-sm" />
        </div>
        <div>
          <p className="font-medium text-gray-900 group-hover:text-[#0194d0] transition-colors">Make Payment</p>
          <p className="text-xs text-gray-500">Pay your bill online</p>
        </div>
      </Link>
      
      {/* Decorative gradient line at bottom */}
      <div className="h-1 bg-[#0194d0]"></div>
    </div>
  </div>
</nav>
...
>
  ...
  <FaChevronRight className="text-[10px] xs:text-xs md:text-sm" />
              ...
              <button 
                ...
                className="lg:hidden w-8 h-8 xs:w-9 xs:h-9 sm:w-10 sm:h-10 bg-[#0194d0] rounded-lg xs:rounded-xl flex items-center justify-center text-white shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
              >
            ...
            <div className="p-6">
              <div className="space-y-3">
                {services.map((service) => (
                  ...
                  >
                    <div className="w-12 h-12 bg-[#0194d0] rounded-xl flex items-center justify-center group-hover:from-[#0194d0] group-hover:to-[#01d09e] transition-all">
                      <service.icon className="text-xl text-white group-hover:text-white/70 transition-colors" />
                    </div>
...
<div className="p-7 max-h-[70vh] overflow-y-auto">
  ...
  >
    <div className="w-10 h-10 bg-[#0194d0]/10 rounded-lg flex items-center justify-center">
      <FaShieldAlt className="text-[#0194d0]" />
    </div>
    <div>
      <span className="font-medium text-gray-900">Insurance</span>
      <p className="text-xs text-gray-500">Accepted plans & payments</p>
    </div>
  </Link>
    ...
    <div>
      ...
      <p className="text-xs text-gray-500">Get in touch</p>
              ...
              <Link
                href="/book"
                onClick={() => setMobileMenuOpen(false)}
                className="mt-6 block w-full bg-[#0194d0] text-white px-6 py-4 rounded-xl font-semibold text-center shadow-lg hover:shadow-xl transition-all"
              >
                Book Appointment
              </Link>
            </div>
